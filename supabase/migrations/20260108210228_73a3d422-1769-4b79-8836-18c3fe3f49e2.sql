-- Función de migración de datos sold_tickets -> orders
...
BEGIN
  ...
  FOR v_order_record IN 
    SELECT 
      ...
      MAX(st.approved_by) as approved_by,
    ...
    WHERE st.payment_reference IS NOT NULL
      AND st.status IN ('reserved', 'pending', 'sold')
  ...
  LOOP
    ...
    -- Insertar en orders (ignorar si ya existe)
    INSERT INTO orders (
